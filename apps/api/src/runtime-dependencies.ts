import { HeadBucketCommand, S3Client } from '@aws-sdk/client-s3';
import { Redis } from 'ioredis';
import pg from 'pg';

import type { ReadinessProbes } from '@vision/observability';
import type { RuntimeConfig, SecretResolver } from '@vision/shared';
import { s3CredentialProvider } from '@vision/shared';

export function createRuntimeDependencies(config: RuntimeConfig, secrets: SecretResolver) {
  const postgres = new pg.Pool({
    connectionString: secrets.resolve('DATABASE_URL'),
    max: 2,
    connectionTimeoutMillis: 1500,
    query_timeout: 1500,
    statement_timeout: 1500,
    idleTimeoutMillis: 5000,
  });

  postgres.on('error', () => {
    // Readiness reports connection failure without exposing raw credentials/errors.
  });

  const redis = new Redis(secrets.resolve('REDIS_URL'), {
    connectTimeout: 1500,
    commandTimeout: 1500,
    maxRetriesPerRequest: 0,
    enableOfflineQueue: false,
    retryStrategy: () => 1000,
  });

  redis.on('error', () => {
    // Readiness reports connection failure.
  });

  const storage = new S3Client({
    endpoint: config.S3_ENDPOINT,
    region: config.S3_REGION,
    forcePathStyle: true,
    maxAttempts: 1,
    credentials: s3CredentialProvider(secrets),
    requestHandler: {
      connectionTimeout: 1500,
      requestTimeout: 1500,
    },
  });

  const probes: ReadinessProbes = {
    postgres: async () => {
      await postgres.query('SELECT 1');
    },
    redis: async () => {
      if ((await redis.ping()) !== 'PONG') {
        throw new Error('REDIS_NOT_READY');
      }
    },
    storage: async () => {
      await storage.send(new HeadBucketCommand({ Bucket: config.S3_BUCKET }), {
        abortSignal: AbortSignal.timeout(1500),
      });
    },
  };

  return {
    probes,
    redis,
    storage,
    async close() {
      redis.disconnect();
      storage.destroy();
      await postgres.end();
    },
  };
}
