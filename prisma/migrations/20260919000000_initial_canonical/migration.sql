...
-- Accepted final-reconciliation constraints (spec-v1.0.2).
ALTER TABLE "Approval" ADD CONSTRAINT "Approval_subject_check" CHECK (
  ("subjectType" = 'CONCEPT' AND "conceptVersionId" IS NOT NULL AND "renderId" IS NULL)
  OR ("subjectType" = 'RENDER' AND "renderId" IS NOT NULL AND "conceptVersionId" IS NULL)
);
CREATE UNIQUE INDEX "KnowledgeSnapshot_one_active_key" ON "KnowledgeSnapshot" ("key")
  WHERE "status" = 'ACTIVE';
ALTER TABLE "AttributionEvent" ADD CONSTRAINT "AttributionEvent_revenue_complete" CHECK (
  "eventType" <> 'REVENUE' OR ("valueAmountMinor" IS NOT NULL AND "valueCurrency" IS NOT NULL)
);
