...
export const ProductFocusWindowSchema = z.object({
  startMs: z.number().int().nonnegative(),
  endMs: z.number().int().positive(),
  assetId: UuidSchema,
  region: NormalizedRectSchema,
...
export const EditorialTextCueSchema = z.object({
  id: z.string().min(1),
  startMs: z.number().int().nonnegative(),
  endMs: z.number().int().positive(),
  text: z.string().min(1),
...
export const PresenterCueSchema = z.object({
  assetId: UuidSchema,
  startMs: z.number().int().nonnegative(),
  endMs: z.number().int().positive(),
  region: NormalizedRectSchema,
...
export const EditingRationaleSchema = z.object({
...
export const EditingPlanSpecSchema = z.object({
...
export const EditingProfileVersionSpecSchema = z.object({
  identity: z.object({
  ...
  pacing: z.object({
    ...
    preferredAverageShotMs: z.object({
    ...
    patternInterruptGuidance: z.object({
      enabled: z.boolean(),
  ...
  motion: z.object({
    ...
    defaultTransitionDurationMs: z.object({
      min: z.number().int().positive(),
      max: z.number().int().positive(),
    }).strict(),
  }).strict(),
  ...
  presenter: z.object({
    ...
    preferredWidthPercent: z.object({
  ...
  hook: z.object({
    ...
    preferredOpeningModes: z.array(z.enum([
      "PRESENTER",
      "PRODUCT_RESULT",
      "PROBLEM_VISUAL",
      "TEXT_STATEMENT",
