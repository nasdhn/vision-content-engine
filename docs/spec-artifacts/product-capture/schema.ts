...
export const BrowserProfileSchema = z.object({
  ...
  recordVideoSize: z.object({
    width: z.number().int().positive(),
    height: z.number().int().positive(),
  }).strict().optional(),
}).strict();

export const CaptureFixturePolicySchema = z.object({
  mode: z.enum([
    "SEEDED_DEMO_DATA",
...
const MarkMomentStep = z.object({
  type: z.literal("MARK_MOMENT"),
  key: z.string().min(1),
  editorialMeaning: z.string().min(1),
}).strict();

const VisualSettleStep = z.object({
...
export const CaptureOutputSpecSchema = z.object({
  key: z.string().min(1),
...
export const CaptureSafetyPolicySchema = z.object({
...
export const CaptureTimeoutPolicySchema = z.object({
...
export const CaptureScenarioVersionSpecSchema = z.object({
  identity: z.object({
...
export const CaptureRunReportSchema = z.object({
  ...
  markedMoments: z.array(z.object({
    key: z.string().min(1),
