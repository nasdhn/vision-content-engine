...
test('authenticated human upload, private preview, selection, last rejection and reselection', async ({
  ...
  expect(cookies.find((c) => c.name === '__Host-vce')).toMatchObject({
    secure: true,
    httpOnly: true,
    sameSite: 'Strict',
  });
  ...
  await page.screenshot({ path: 'test-results/recording-pack-desktop.png', fullPage: true });
});
test('mobile layout and keyboard login, validation feedback on unreadable drop upload', async ({
  ...
  await page.locator('.drop').dispatchEvent('drop', { dataTransfer: data });
  await expect(page.getByRole('alert')).toContainText('illisible');
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(
    true,
  );
  await page.screenshot({ path: 'test-results/recording-pack-mobile.png', fullPage: true });
});
