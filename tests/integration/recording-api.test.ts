...
it('creates one shared secure HttpOnly same-site authenticated session', async () => {
  ...
  expect(
    (
      await fetch(`${base}/api/recording-packs`, {
...
it('rejects both cross-origin and missing-CSRF writes before application mutation', async () => {
  ...
  for (const headers of [
    ...
    const response = await fetch(`${base}/api/recordings/${randomUUID()}/selection`, {
      method: 'POST',
      headers: {
        ...headers,
        Cookie: cookie,
...
it('uses the authenticated actor, validates IDs, and hides internal errors', async () => {
  ...
  const response = await fetch(`${base}/api/recording-packs`, {
    headers,
  });
