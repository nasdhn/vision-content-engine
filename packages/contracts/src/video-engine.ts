import { EditingPlanSpecSchema, RenderSettingsSchema } from './editing.js';
import { z } from 'zod';
      ...
      .object({
        ...
        color: z
          .object({
            ...
            hdrKind: z.enum(['SDR', 'HDR10', 'HLG', 'DOLBY_VISION', 'UNKNOWN_HDR']).optional(),
          })
          .strict()
          .optional(),
      })
...
export const TemplateSlotSchema = z
  .object({
    key: z.string().min(1),
    accepts: z.enum(['VIDEO', 'IMAGE', 'AUDIO', 'TEXT', 'PRESENTER', 'PRODUCT']),
    required: z.boolean(),
...
export const TemplateRuntimeContractSchema = z
  .object({
    ...
    supportedCanvas: z
      ...
      .strict(),
    supportedLayers: z
      .array(z.enum(['BACKGROUND', 'PRODUCT', 'BROLL', 'PRESENTER', 'TEXT', 'CAPTION', 'OVERLAY']))
      .min(1),
...
export const CodecProfileSchema = z
  .object({
    key: z.string().min(1),
    version: z.string().min(1),
...
export const ResolvedRenderAssetSchema = z
  .object({
    assetId: UuidSchema,
...
export const TechnicalQaReportSchema = z
  .object({
